...
static const float CrystalCoordinates[5][4] =
{
	{ -9426.8515f, 1958, 85.688f, 2.353f },
	{ -9368.7676f, 1939.82f, 85.556f, 5.7402f },
	{ -9328.207f, 1887.354f, 85.556f, 5.7402f },
	{ -9390.403f, 2008.722f, 85.556f, 5.7402f },
	{ -9392.772f, 1942.075f, 85.556f, 5.7402f },
};
...
struct boss_ossirianAI : public ScriptedAI
{
	...
	void CleanCrystal()
	{
		while (true)
		{
			if (auto creature = GetClosestCreatureWithEntry(m_creature, NPC_OSSIRIAN_TRIGGER, 500.0f))
			{
		...
		while (true)
		{
			if (auto gameobject = GetClosestGameObjectWithEntry(m_creature, GO_OSSIRIAN_CRYSTAL, 500.0f))
			{
				gameobject->Delete();
			}
			else
	...
	void DoSpawnNextCrystal()
	{
		...
		if (CrystalIterator == 5)
			CrystalIterator = 0;
		Creature* pOssirianTrigger = NULL;
		if (pOssirianTrigger = m_creature->SummonCreature(NPC_OSSIRIAN_TRIGGER, CrystalCoordinates[CrystalIterator][0], CrystalCoordinates[CrystalIterator][1], CrystalCoordinates[CrystalIterator][2], CrystalCoordinates[CrystalIterator][3], TEMPSUMMON_CORPSE_DESPAWN, 0))
		{
			if (pCrystal = GetClosestGameObjectWithEntry(pOssirianTrigger, GO_OSSIRIAN_CRYSTAL, 500.0f))
	...
	void UpdateAI(const uint32 uiDiff) override
	{
		if (!m_creature->SelectHostileTarget() || !m_creature->getVictim())
			return;
		if (m_gobjectchecktimer <= uiDiff)
		{
			if (pCrystal != NULL)
			{
				if (!GetClosestCreatureWithEntry(pCrystal, NPC_OSSIRIAN_TRIGGER, 10.0f) && pCrystal->getLootState() == GO_ACTIVATED)
				{
					pCrystal->Delete();
					pCrystal = NULL;
					DoSpawnNextCrystal();
...
bool GOUse_go_ossirian_crystal(Player* /*pPlayer*/, GameObject* pGo)
{
	if (Creature* pOssirianTrigger = GetClosestCreatureWithEntry(pGo, NPC_OSSIRIAN_TRIGGER, 10.0f))
		pOssirianTrigger->CastSpell(pOssirianTrigger, aWeaknessSpell[urand(0, 4)], true);
	return true;
}
